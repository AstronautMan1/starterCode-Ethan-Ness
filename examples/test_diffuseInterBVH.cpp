...
void interDiffuse(int argc, char *argv[]){
    ...
    Light mainlight(point3(5, 5, 5), vec3(1, 1, 1), "pointlight"); // light object at position 5,5,5 with white light color
        ...
        for(int y = 0; y < fb.getHeight(); ++y){ 
            ...
            for (int p = 0; p < rpp_NSquare; ++p){
                for (int q = 0; q < rpp_NSquare; ++q){
                    ...
                    color += sceneBVH->computeRayColor(r, tmin, tmax, mainlight, vec3(0,0,0), depth);
                }
            }
